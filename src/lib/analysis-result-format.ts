  ...
  return [
    ...
    result.vacancyFit && 'candidateLevelFit' in result.vacancyFit
      ? [
          ...
          result.vacancyFit.requiredSkills.length
            ? markdownList('Обязательные навыки', result.vacancyFit.requiredSkills)
            : '',
          result.vacancyFit.optionalSkills.length
            ? markdownList('Дополнительные навыки', result.vacancyFit.optionalSkills)
            : '',
          result.vacancyFit.missingSkills.length
            ? markdownList('Не найдено в резюме', result.vacancyFit.missingSkills)
            : '',
          result.vacancyFit.risks.length ? markdownList('Риски', result.vacancyFit.risks) : '',
          result.vacancyFit.probableRejectionReasons.length
            ? markdownList('Возможные причины отказа', result.vacancyFit.probableRejectionReasons)
            : '',
        ]
          .filter(Boolean)
          .join('\n\n')
