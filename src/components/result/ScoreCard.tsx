...
export function ScoreBreakdownDetails({ title, breakdown }: { title: string; breakdown: ScoreBreakdown }) {
  return <Collapsible>
    <CollapsibleTrigger className="flex w-full items-center justify-between rounded-md py-1 text-left text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
      Почему {breakdown.score}/100?<ChevronDown aria-hidden="true" className="h-4 w-4" />
    </CollapsibleTrigger>
    <CollapsibleContent>
      <div className="mt-2 space-y-3 border-t pt-3" aria-label={`Расчёт: ${title}`}>
        {breakdown.components.map((component, index) => <div key={`${component.name}-${index}`} className="grid grid-cols-[1fr_auto] gap-x-3 text-sm">
          <span className="font-medium">{humanizeKey(component.name)}</span>
          <span>{component.score} × {Math.round(component.weight * 100)}% = {component.contribution}</span>
          {component.explanation && <span className="col-span-2 text-xs text-muted-foreground">{component.explanation}</span>}
        </div>)}
      </div>
    </CollapsibleContent>
  </Collapsible>;
