...
describe("Spring API client", () => {
  ...
  it("serializes all current vacancy filters and technologies", async () => {
    server.use(http.get(`${API_URL}/api/vacancies`, ({ request }) => {
      const params = new URL(request.url).searchParams;
      expect(Object.fromEntries(params)).toMatchObject({
        query: "Java Backend", area: "Москва", employer: "Acme", level: "MIDDLE",
        workFormat: "REMOTE", salaryFrom: "150000", salaryTo: "250000", currency: "RUR",
        salaryOnly: "true", publishedFrom: "2026-08-01", sort: "DATE", page: "0", pageSize: "50",
      });
    ...
    await expect(searchVacancies({
      ...criteria, area: "Москва", employer: "Acme", level: "MIDDLE", workFormat: "REMOTE",
      salaryFrom: 150000, salaryTo: 250000, currency: "rur", salaryOnly: true,
      technologies: ["Java", "Spring Boot"], publishedFrom: "2026-08-01", sort: "DATE", pageSize: 50,
    })).resolves.toMatchObject({ totalElements: 0, warnings: [] });
  });

  it("uses top-level pagination fields", async () => {
    server.use(http.get(`${API_URL}/api/vacancies`, () => HttpResponse.json({
      items: [], page: 2, pageSize: 20, totalPages: 7, totalElements: 133,
      pagination: { page: 99 }, warnings: ["Количество приблизительное"],
    })));
    await expect(searchVacancies({ ...criteria, page: 2 })).resolves.toMatchObject({
      page: 2, pageSize: 20, totalPages: 7, totalElements: 133,
    });
  });
  ...
  it("loads vacancy details only through the details endpoint", async () => {
    let searches = 0;
    server.use(
      http.get(`${API_URL}/api/vacancies`, () => { searches += 1; return HttpResponse.json({ items: [], page: 0, pageSize: 20, warnings: [] }); }),
      http.get(`${API_URL}/api/vacancies/hh-123`, () => HttpResponse.json({ id: "hh-123", title: "Java", skills: [], requirements: [], responsibilities: [] })),
    );
  ...
  it("passes AbortSignal to vacancy requests", async () => {
    ...
    const spy = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({ items: [], page: 0, pageSize: 20, warnings: [] }), { status: 200, headers: { "Content-Type": "application/json" } }));
  ...
  it("preserves top-level vacancy error codes and maps 429", async () => {
    server.use(http.get(`${API_URL}/api/vacancies`, () => HttpResponse.json({ code: "RATE_LIMITED", message: "raw" }, { status: 429 })));
  ...
  it("rejects successful responses that are not JSON", async () => {
    ...
    await expect(getApiJson("/api/health")).rejects.toMatchObject({ message: "Backend returned an invalid JSON response", status: 200 });
  });
});
