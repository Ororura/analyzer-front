...
describe('resume backend client', () => {
  ...
  it('loads provider configuration from the backend', async () => {
    server.use(
      http.get(AI_PROVIDERS_URL, ({ request }) => {
  ...
  it('posts the PDF as multipart and sends provider/profile as query parameters', async () => {
    ...
    server.use(
      http.post(RESUME_ANALYZE_URL, async ({ request }) => {
  ...
  it.each([
    ...
    await expect(
      analyzeResume(file, { provider: 'CODEX_CLI', profile: 'JAVA_BACKEND', analysis }),
    ).resolves.toBeTruthy();
  });
  ...
  it('passes AbortSignal to fetch', async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify(resumeAnalysisResult), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    );
    await analyzeResume(new File(['pdf'], 'resume.pdf'), {
      provider: 'POLZA',
  ...
  it('preserves structured backend error code and status', async () => {
    ...
    await expect(
      analyzeResume(new File(['pdf'], 'resume.pdf'), { provider: 'CODEX_CLI', profile: 'JAVA_BACKEND' }),
  ...
  it('rejects a successful response that does not match the analysis schema', async () => {
    ...
    await expect(
      analyzeResume(new File(['pdf'], 'resume.pdf'), {
        provider: 'CODEX_CLI',
        profile: 'JAVA_BACKEND',
      }),
  ...
  it('parses the structured v2 response without dropping nested analysis fields', async () => {
    ...
    const result = await analyzeResume(new File(['pdf'], 'resume.pdf'), {
      provider: 'CODEX_CLI',
      profile: 'JAVA_BACKEND',
    });
...
describe('backend error messages', () => {
  const cases = [
    ['INVALID_FILE', 'Не удалось обработать'],
    ['INVALID_QUERY', 'Параметры запроса'],
    ['PDF_TOO_LARGE', 'PDF превышает'],
    ['RESUME_TEXT_TOO_LARGE', 'Текст резюме'],
    ['UNSUPPORTED_FILE_TYPE', 'только PDF'],
    ['PDF_PARSE_FAILED', 'прочитать PDF'],
    ['EMPTY_RESUME', 'не найден текст'],
    ['AI_PROVIDER_FAILED', 'завершил анализ с ошибкой'],
    ['AI_INVALID_RESPONSE', 'некорректный результат'],
    ['AI_PROVIDER_UNAVAILABLE', 'сейчас недоступен'],
    ['AI_PROCESS_START_FAILED', 'не удалось запустить'],
    ['AI_RATE_LIMITED', 'ограничил количество'],
    ['AI_TIMEOUT', 'слишком много времени'],
    ['ANALYSIS_FAILED', 'выполнить анализ'],
    ['INVALID_SELECTION', 'выбранные вакансии'],
    ['SELECTION_TOO_LARGE', 'Максимум — 200'],
    ['VACANCY_NOT_FOUND', 'больше недоступна'],
    ['VACANCY_RATE_LIMITED', 'ограничил количество'],
    ['VACANCY_PROVIDER_TIMEOUT', 'не ответил вовремя'],
    ['VACANCY_PROVIDER_FAILED', 'временно недоступен'],
  ...
  it.each([
    ...
    [502, 'некорректный ответ'],
    [503, 'временно недоступен'],
    [504, 'слишком много времени'],
